...
const customPlaceholders = {
  ...
  outfitShape: "例：カントリー風ワンピース、寄宿学校風制服、ふわふわケープ",
  outfitSeason: "例：梅雨向け、真夏、クリスマス前、春のおでかけ",
  outfitDecor: "例：袖だけフリル、胸元に大きなリボン、裾にレース",
  fruitPattern: "例：苺と白い小花、さくらんぼ刺繍、桃柄",
  flowerPattern: "例：淡い薔薇柄、白い小花柄、デイジーの刺繍",
  otherPattern: "例：ギンガムチェック、薔薇入りストライプ、細かい水玉",
...
function buildPrompt({ locationType, locationOption, selected, custom, outdoorWorldId, indoorWorldId }) {
  ...
  const outfitSeason = joinValues(selected, custom, "outfitSeason", "おまかせ");
  const outfitShape = joinValues(selected, custom, "outfitShape", "おまかせ");
  const outfitDecor = joinValues(selected, custom, "outfitDecor", "おまかせ");
  const fruitPattern = joinValues(selected, custom, "fruitPattern", "おまかせ");
  const flowerPattern = joinValues(selected, custom, "flowerPattern", "おまかせ");
  const otherPattern = joinValues(selected, custom, "otherPattern", "おまかせ");
  const outfitColor = joinValues(selected, custom, "outfitColor", "おまかせ");
  const outfitForRule = [outfitShape, outfitDecor, fruitPattern, flowerPattern, otherPattern, outfitColor].filter(Boolean).join("、");
  ...
  const isAnimalCostume =
    animalCostumeKeywords.some((keyword) => outfitForRule.includes(keyword)) ||
    /(?:きぐるみ|キグルミ|着ぐるみ|着ぐるみ服|被り物|小動物風)/.test(outfitForRule);
  const isBoardingSchoolOutfit =
    boardingSchoolKeywords.some((keyword) => outfitForRule.includes(keyword)) ||
    /(?:寄宿学校|学院|制服|ロングコート|ブレザー|黒リボン|学生風)/.test(outfitForRule);
  ...
  const toggleOption = (categoryId, option, single = false, maxSelect = null) => {
    ...
    setSelected((prev) => {
      ...
      if (single) {
        ...
        if (option === "おまかせ") return { ...prev, [categoryId]: ["おまかせ"] };
    ...
    <div className="page">
      ...
      <div className="container">
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="hero">
          ...
          <div className="update-time">最終更新：2026/05/29 13:38</div>
