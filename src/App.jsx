...
function getSizeInstruction(size) {
  ...
  if (size.includes("1:1") || size.includes("正方形")) {
    return `縦横比は必ず1:1の正方形画像にしてください。縦長や横長にはしないでください。ペットの高さは画面縦の38〜48％程度を目安にしてください。${commonRule}`;
  }
  if (size.includes("4:5")) {
    return `縦横比は必ず4:5の縦長画像にしてください。正方形（1:1）や横長にはしないでください。ペットの高さは画面縦の35〜45％程度を目安にしてください。${commonRule}`;
  }
  if (size.includes("9:16")) {
    return `縦横比は必ず9:16の縦長画像にしてください。正方形（1:1）や横長にはしないでください。ペットの高さは画面縦の25〜33％程度を目安にしてください。${commonRule}`;
  }
  if (size.includes("16:9")) {
    return `縦横比は必ず16:9の横長画像にしてください。正方形（1:1）や縦長にはしないでください。ペットの高さは画面縦の40〜55％程度を目安にしてください。${commonRule}`;
  }
