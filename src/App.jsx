...
function buildIndoorScene({ indoorWorldId, locationOption, customLocation }) {
  ...
  const dreamLolitaPlaceDescriptions = {
    ...
    "お姫様のバスタイム":
      "優雅なお姫様のバスルーム。白いアンティーク猫足バスタブの泡風呂で、ペットは頭の後ろ側を湯船のへりにもたれかけ、泡風呂の中で気持ちよさそうにうとうと眠っている場面。体はお湯と泡の中に沈み、顔と首まわりだけが自然に見えています。前足は浴槽の縁に乗せず、泡の中に自然に隠してください。人間のように身を乗り出したり、浴槽を掴んだりしないでください。しっぽは無理に見せず、元写真の向きや体勢的に自然な場合のみ、泡の外に少し見えてもよいです。レースのカーテン、花柄の壁紙、上品なアンティーク家具、白く美しい猫足の置き台、シャンプーボトルや香水瓶が見える、夢かわいく清潔感のあるバスタイム空間。",
    "苺スウィーツルーム":
...
function buildPrompt({ locationType, locationOption, selected, custom, outdoorWorldId, indoorWorldId }) {
  ...
  const itemSentence = isBathScene
    ? items
      ? `選んだ小物（${items}）は、泡風呂やバスルームに自然になじむ、浮かべられる小物として控えめに取り入れてください。ペットの顔と首まわりを邪魔しない量と位置にしてください。`
    ...
    <div className="page">
      ...
      <div className="container">
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="hero">
          ...
          <div className="update-time">最終更新：2026/06/01（月） 07:38頃</div>
